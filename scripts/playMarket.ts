...
async function main() {
  ...
  const NFT = await ethers.getContractFactory("HxToken")
  const nft = await NFT.deploy(_name,_symbol)
  const tokenAddress = nft.getAddress()
...
async function parseItems(items:any,nft:any) {
