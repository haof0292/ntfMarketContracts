...
describe("NFTMarketplace", function () {
  ...
  describe("basic function test:", function (){
    it("Should create market item successfully", async function() {
      ...
      await nft.mintTo(address0, tokenId);  //tokenId=1
      await nft.approve(market.getAddress(), tokenId);
      await market.createMarketItem(nft.getAddress(), tokenId, auctionPrice, { value: listingFee });
    ...
    it("Should create market item with EVENT", async function() {
      const {nft, market, listingFee, account0} = await loadFixture(delpoyMarketFixture);
      const address0 = await account0.getAddress();
      await nft.mintTo(address0, tokenId)  
      await nft.approve(market.getAddress(),tokenId)
      await expect(market.createMarketItem(nft.getAddress(), tokenId, auctionPrice, { value: listingFee }))
    ...
    it("Should create market item and buy (by address#1) successfully", async function() {
      ...
      await expect(market.connect(account1).createMarketSale(nft.getAddress(), tokenId, { value: auctionPrice}))
      ...
      expect(await nft.ownerOf(tokenId)).to.be.equal(address1)
    ...
    it("Should revert buy if seller transfer the token out", async function() {
      ...
      await nft.transferFrom(address0,address2,tokenId)
    ...
    it("Should revert to delete(de-list) with wrong params", async function() {
      ...
      //not a correct id
      await expect(market.deleteMarketItem(100)).to.be.reverted
  
      //not owner
      await expect(market.connect(account1).deleteMarketItem(tokenId)).to.be.reverted
  
      await nft.transferFrom(address0,address1,tokenId)
      //not approved to market now
      await expect(market.deleteMarketItem(tokenId)).to.be.reverted
    })
    ...
    it("Should create market item and delete(de-list) successfully", async function() {
      ...
      await market.createMarketItem(nft.getAddress(), tokenId, auctionPrice, { value: listingFee })
      await market.deleteMarketItem(tokenId)
  
      await nft.approve(ethers.ZeroAddress, tokenId)
  
      // should revert if trying to delete again
      await expect(market.deleteMarketItem(tokenId))
        .to.be.reverted
    })
    ...
    it("Should seller, buyer and market owner correct ETH value after sale", async function() {
      ...
      await nft.connect(account1).mintTo(address1, tokenId)  //tokenId=1
      await nft.connect(account1).approve(market.getAddress(),tokenId)
  
      let sellerBalance = await ethers.provider.getBalance(address1)
      const txresponse = await market.connect(account1).createMarketItem(nft.getAddress(), tokenId, auctionPrice, { value: listingFee })
      ...
      // console.log("+++++++++++ now tokenId:", tokenId);
